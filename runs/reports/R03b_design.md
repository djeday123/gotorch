...
| MatMul (F32, batch=1) | direct | **разница режимов документирована** | rel ≤ 1e-2 hybrid (abs=5e-2 + rel=1e-2·\|ref\|) | **Предположение bit-exact опровергнуто фактом** (impl-4-final): goml legacy cublas handle = TF32 (`goml/backend/cuda/cublas.go:216`, `cublasSetMathMode(TF32)`), gotorch = pedantic FP32 (`gotorch/v6/cuda/cublas_purego.go:5-9`, R02b Ворот 2 решение). Расхождение rel ≤ 1e-3 на elem-wise, hybrid покрывает cancellation при \|ref\|→0. **Adapter-путь ~3600× ТОЧНЕЕ legacy** (impl-4-final Sверка 3.2 — измерено). Это разница режимов, не баг раскладки — доказано bit-exact 256/256 через MatMulF32_TF32 sверкой 3.4 (adapter в том же TF32-mode что fb = bit-exact identical). Исправление ошибочной посылки, не расширение допуска — та же грань, что MatMul-линейка impl-3 (абс → rel), не «допуск задним числом». Правило переходного периода: gotorch остаётся FP32 pedantic (ARCHITECTURE.md), выравнивание gotorch под legacy TF32 отклонено. При полном переходе трейна на gotorch — разница исчезает в пользу FP32. |
